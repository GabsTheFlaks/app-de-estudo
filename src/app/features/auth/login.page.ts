    ...
    <div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden bg-slate-50 dark:bg-[#0f172a]">
      ...
      <div class="max-w-md w-full bg-white/90 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200/50 dark:border-white/10 p-10 rounded-3xl shadow-[0_20px_50px_-12px_rgba(0,0,0,0.1)] dark:shadow-2xl relative z-10">
        ...
        <div class="mb-10 text-center w-full">
          <h2 class="mt-2 text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Bem-vindo</h2>
          <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">Faça login para acessar seus cursos</p>
        </div>
        ...
        <form [formGroup]="loginForm" (ngSubmit)="onSubmit()" class="w-full space-y-6">
          ...
          <div class="text-center mt-4">
            <span class="text-sm text-slate-500 dark:text-slate-400">Não tem uma conta? </span>
            <a routerLink="/register" class="text-sm font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors hover:underline">Cadastre-se</a>
          </div>
        </form>
      </div>
    </div>
