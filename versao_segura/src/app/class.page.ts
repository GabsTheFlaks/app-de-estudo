...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterLink, MatIconModule, DatePipe, MatDialogModule],
  template: `
      ...
      @if (isLoading()) {
        ...
        <div class="mt-6 flex flex-col lg:flex-row gap-6 relative items-start">
          ...
          <div class="flex-1 space-y-4 w-full order-1 lg:order-2">
            ...
            @if (lessons().length === 0 && !course()!.link_drive && appUser()?.role !== Role.ADMIN) {
...
export class ClassPage implements OnInit {
  ...
  async editLegacy() {
    ...
    const dialogRef = this.dialog.open(PromptDialogComponent, {
      width: '400px',
      data: {
        title: 'Editar Material Principal',
        message: 'Digite o novo link para o material principal:',
        label: 'Link',
        initialValue: this.course()?.link_drive || ''
      }
    });

    dialogRef.afterClosed().subscribe(async (newLink) => {
